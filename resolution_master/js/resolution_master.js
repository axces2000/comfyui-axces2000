...
// Aspect ratio display shapes (w:h in relative units)
const SHAPE = {
  landscape: { w: 48, h: 30 },
  square:    { w: 36, h: 36 },
  portrait:  { w: 30, h: 48 },
...
const WIDGET_H    = 148; // total custom widget height
const NODE_W      = 310;
...
function createResolutionWidget(node) {
  ...
  // Remove default combo widgets generated by ComfyUI and replace with ours
  ...
  // ── Sync back to the hidden combo widgets so Python receives correct values ──
  function syncCombos() {
    const resW   = node.widgets?.find(w => w.name === "resolution");
    const orientW = node.widgets?.find(w => w.name === "orientation");
    if (resW   && resW   !== widget) resW.value   = selectedRes;
    if (orientW && orientW !== widget) orientW.value = selectedOrient;
    widget.value = `${selectedRes}_${selectedOrient}`;
  ...
  widget.draw = function (ctx, node, width, y) {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.roundRect(px + 10, previewY, halfW - 4, 22, 5);
    ctx.fill(); ctx.stroke();
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.roundRect(hbX, previewY, halfW - 4, 22, 5);
    ctx.fill(); ctx.stroke();
  ...
  widget.mouse = function (event, pos, node) {
    ...
    // Dropdown click — show a LiteGraph context menu with resolution options
    if (widget._ddRect) {
  ...
  widget._restore = function (val) {
    // val may come as "2K (1080p)_landscape" or separately
    if (!val) return;
    const lastUnder = val.lastIndexOf("_");
    if (lastUnder === -1) return;
    const res    = val.slice(0, lastUnder);
    const orient = val.slice(lastUnder + 1);
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      // Remove the two auto-generated combo widgets
      ["resolution", "orientation"].forEach(name => {
